...
public class FindJunctionAction {
    ...
    private GeometryActions geometry;
...
//    int counter;
//    int minCounter = 1;
    double[] distanceAtCount;
    int[] ticksAtCount;
    int totalTicks;
    int overshoot;
    int driveSpeed = 350;
    int idCounter;
//    double prevDistBeforeDetection;
//    int prevDistTicksBeforeDetection;
//    double prevDistAfterDetection;
//    int prevDistTicksAfterDetection;
...
//    int minDistance = 300;
    ...
    public FindJunctionAction(HardwareMap hardwareMap, Telemetry telemetry, LinearOpMode opMode, DriveActions driveActions, AttachmentActions attachmentActions, DistanceSensorActions distanceSensorActions, EncoderActions encoderActions, GyroActions gyroActions, GeometryActions geometry) {
...
//        distanceAtCount = new double[minCounter];
//        ticksAtCount = new int[minCounter];
        opModeObj = opMode;
    }
...
//            encoderActions.setVelocity(speed, speed, speed, speed);
            ...
            double currentDistance = s1.getSensorDistance(DistanceUnit.MM); //Get the distance only once per cycle
            RobotLog.dd("FindJunction", "distance: %f, currentPos: %d, counter: %d, time: %d, velocity: %f, heading error %f", currentDistance, currentPosition, geometry.counter, System.currentTimeMillis(), currentVelocity, tiltError / (speed * 0.05));
//            if (currentDistance > 2000) { //System for throwing out errors. If it goes from more than 2000 to less than 2000 to more again in less than three cycles, we know it's an error
//                if (idCounter <= 2) {
//                    dist = prevDistBeforeDetection;
//                    ticksAtLowestDist = prevDistTicksBeforeDetection;
//                }
//                idCounter = 0;
//            }
//            if (currentDistance > minDistance && idCounter > minCounter) {
...
//                    prevDistBeforeDetection = dist;
//                    prevDistTicksBeforeDetection = ticksAtLowestDist;
//                }
//                idCounter++;
//
//                if (currentDistance < dist) { //If the current distance is less than the past minimum, it's the new minimum and we record where we were when we got it
//
//                    prevDistAfterDetection = dist;
//                    prevDistTicksAfterDetection = ticksAtLowestDist;
//                    dist = currentDistance;
//                    ticksAtLowestDist = currentPosition;
//
//                }
//
//                if (currentDistance < minDistance) { //If we've seen the pole
//
//                    memBitOn = true;
//
//                }
//
//                if ((memBitOn == true) && (currentDistance > dist)) { //If we've seen the pole and our distance is above the minimum we've recorded, increment the counter
//
//                    distanceAtCount[counter] = currentDistance;
//                    ticksAtCount[counter] = currentPosition;
//                    counter = counter + 1;
//
//                } else {
//                    counter = 0;
...
//
//                    if (isError(distanceAtCount, ticksAtCount)) {
//                        int lowest = getLowest(distanceAtCount, prevDistAfterDetection);
//                        if (lowest < 0) {
//                            dist = prevDistAfterDetection;
//                            ticksAtLowestDist = prevDistTicksAfterDetection;
//                        } else {
//                            dist = distanceAtCount[lowest];
//                            ticksAtLowestDist = ticksAtCount[lowest];
//                        }
//                    } else {
//                        targetPos = ticksAtLowestDist;
//                        encoderActions.setVelocity(0, 0, 0, 0);
...
//            }
            ...
            if (geometry.counter <= geometry.minCounter && keptGoingCounter < 6) { //If we've reached the set position but haven't seen the pole, we try again but go a little further
                if (targetPos < 0) {
                    targetPos -= 60;
                } else {
                    targetPos += 60;
        ...
        if (state == 2) { //If we've seen the goal
            ...
            telemetry.addData("counter", geometry.counter);
...
//            opModeObj.sleep(1000000);
    ...
    public boolean placeOnJunction (double sensorDistance, int ticksAtLowestDist, double degrees, boolean turnTableLeft, int direction, double offset2) {
        if (placeState == 0) {
            if (sensorDistance > geometry.minDistance) { //If something's gone wrong and we haven't seen the junction, we give up and just drop the cone
                placeState = 4;
                return true;
            }
...
//        counter = 0;
        ramp = 0.0;
//        idCounter = 0;
//        prevDistBeforeDetection = 8190;
//        prevDistTicksBeforeDetection = 8190;
        keptGoingCounter = 0;
    }
}
